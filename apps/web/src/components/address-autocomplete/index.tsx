...
import { Delete, Loader2, MapPin, Pencil } from "lucide-react";
...
function AddressAutoCompleteInput(props: CommonProps) {
  ...
  const { data, isLoading } = useSWR(
    debouncedSearchInput.trim().length >= 2
      ? `/api/address/autocomplete?input=${encodeURIComponent(debouncedSearchInput)}`
      : null,
    fetcher
  );
                    ...
                    {predictions.map(
                      (prediction: {
                        ...
                        >
                          ...
                          <span className="text-sm">{prediction.placePrediction.text.text}</span>
                        </CommandPrimitive.Item>
                      )
                    )}
                  </>
                ...
                <CommandEmpty>
                  {!isLoading && predictions.length === 0 && (
                    <div className="py-4 flex items-center justify-center text-sm text-muted-foreground">
                      {debouncedSearchInput.trim().length < 2
                        ? "Type to search for any address, hotel or villa…"
                        : "No results found"}
                    </div>
                  )}
                </CommandEmpty>
