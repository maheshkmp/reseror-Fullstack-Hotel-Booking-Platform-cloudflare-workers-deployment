    ...
    <div className="h-full flex-1 relative">
      ...
      {selectedFiles.length > 0 && (
        <Card className="w-full h-full min-h-[300px] rounded-sm px-4 py-3 shadow-none relative overflow-hidden transition-all duration-200 flex flex-col border-secondary/90">
          {/* Header */}
          <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between border-b border-border/40 pb-3 mb-1">
            <div>
              <h4 className="text-sm font-medium">
                Selected Files <span>{`(${selectedFiles.length})`}</span>
              </h4>
              <p className="text-xs text-muted-foreground mt-1">
                Images are ready. Click <strong className="text-primary font-semibold">Upload All</strong> to complete.
              </p>
            </div>
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              <div className="relative">
                {!isUploading && selectedFiles.length > 0 && (
                  <span className="absolute -inset-1 rounded-sm bg-primary/40 animate-ping opacity-75"></span>
                )}
                <Button
                  className={`cursor-pointer rounded-sm min-w-28 text-xs relative ${
                    !isUploading && selectedFiles.length > 0 ? "shadow-lg shadow-primary/25" : ""
                  }`}
                  icon={
                    isUploading ? (
  ...
  return (
    ...
    >
      ...
      {file.uploadStatus !== "pending" && (
        <div className="absolute top-0 left-0 w-full h-full bg-black/75 text-white flex items-center justify-center z-30 flex-col gap-2 p-2 text-center">
          {file.uploadStatus === "uploading" && (
            <>
              <Loader2 className="size-10 animate-spin text-blue-400" strokeWidth={2} />
              <div className="space-y-1">
                <p className="text-[10px] font-bold uppercase tracking-widest text-blue-400">
                  {file.statusLabel || "Processing..."}
                </p>
          ...
          {file.uploadStatus === "success" && (
            <>
              <CheckCircle2 className="size-12 text-emerald-400" strokeWidth={2} />
              <p className="text-[10px] font-bold uppercase tracking-widest text-emerald-400">Complete</p>
            </>
          )}
          {file.uploadStatus === "error" && (
            <div className="flex flex-col items-center gap-1">
              <XIcon className="size-12 text-red-500" strokeWidth={2} />
              <p className="text-[9px] font-bold text-red-400 uppercase leading-tight">Failed</p>
              <p className="text-[8px] text-white/60 line-clamp-2">{file.error}</p>
            </div>
          )}
        </div>
      )}
