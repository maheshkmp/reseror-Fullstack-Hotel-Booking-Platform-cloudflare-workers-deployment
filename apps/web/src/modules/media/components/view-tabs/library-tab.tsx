...
export function LibraryTab({ selectionMode = false }: LibraryTabProps) {
  ...
  return (
    <div className="h-full flex-1 relative flex flex-col">
      ...
      {selectionMode && (
        <div className="flex items-center gap-3 px-4 py-2.5 bg-primary/8 border-b border-primary/20 text-sm">
          <div className="flex items-center justify-center size-7 rounded-full bg-primary/15 shrink-0">
            <MousePointerClick className="size-4 text-primary" />
          </div>
          <div>
            <span className="font-semibold text-primary">Click images below to select them</span>
            <span className="text-muted-foreground ml-1">— then press <strong className="text-foreground">Use Selected</strong> to add them.</span>
          </div>
        </div>
      )}
